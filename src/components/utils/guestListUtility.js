  ...
  if (isMobileView) {
    return (
      <p>
        {
          <>
            <p className="font-pop text-green-500">
              {rsvpCounts.attending} <span className="font-pop text-xs text-white">Attending</span>
            </p>
          </>
        }
        {
          <>
            <p className="font-pop text-red-500">
              {rsvpCounts.declined} <span className="font-pop text-xs text-white">Declined</span>
            </p>
          </>
        }
        {rsvpCounts.noResponse > 0 && (
          <>
            <p className="font-pop text-white">
              {rsvpCounts.declined} <span className="font-pop text-xs text-white">No Response</span>
            </p>
          </>
        )}
      </p>
    ...
    return (
      <p>
        {
          <>
            <span className="font-pop text-xl text-green-500">{rsvpCounts.attending} </span>
            <span className="font-pop text-xs text-white">
              {pluralize(rsvpCounts.attending, 'Attending')} Attending &nbsp;&nbsp;
            </span>
          </>
        }
        {
          <>
            <span className="font-pop text-xl text-red-500">{rsvpCounts.declined} </span>
            <span className="font-pop text-xs text-white">
              {pluralize(rsvpCounts.declined, 'Declined')} Declined &nbsp;&nbsp;
            </span>
          </>
        }
        {rsvpCounts.noResponse > 0 && (
          <>
            <span className="font-pop text-xl text-white">{rsvpCounts.noResponse} </span>
            <span className="font-pop text-xs text-white">
              {pluralize(rsvpCounts.noResponse, 'No Response')} &nbsp;&nbsp;
            </span>
          </>
        )}
      </p>
...
export const weddingDetails = () => (
  <div
    className="text-center px-16 pt-6 pb-6 ring-1 ring-gray-900/5 sm:mx-auto sm:max-w-4xl sm:rounded-lg sm:px-20"
    style={{ backgroundColor: 'rgb(238, 238, 238)' }}
  >
    <h3 className="text-xl text-black font-bodo mb-4">Kindly Reply by Monday</h3>
    <h3 className="text-xl text-black font-bodo mb-4">March 9, 2026</h3>
  </div>
)
...
export function formatGuestNamesInTable(guest) {
  ...
  if (guest.firstName || guest.lastName) {
    names.push(
      `${guest.title || ''} ${guest.firstName || ''} ${guest.lastName || ''} ${guest.suffix || ''}`.trim()
    )
  }
  ...
  if (guest.partner?.firstName || guest.partner?.lastName) {
    names.push(
      `${guest.partner.title || ''} ${guest.partner.firstName || ''} ${guest.partner.lastName || ''}`.trim()
    )
  }
