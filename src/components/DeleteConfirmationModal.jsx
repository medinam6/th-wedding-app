...
const DeleteConfirmationModal = ({ isOpen, onClose, onConfirm, guestName }) => {
  return (
    <AlertDialog open={isOpen}>
      <AlertDialogContent style={{ backgroundColor: 'rgb(238, 238, 238)' }}>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="font-pop text-black">
            ...
            <span className="font-bold text-red-500">{guestName}</span> and all guest information?
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <Button
            onClick={onClose}
            className="rounded-md border border-blackbg-white px-4 py-2 text-sm font-pop text-black shadow-sm hover:bg-gray-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
